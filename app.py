...
from tool.auth import Cookie
from tool.auth import Auth
from tool.helper import *
from tool.voice_toolkit import voice_toolkit
from common.decorator import handle_exception
import extra_streamlit_components as stx

st.set_page_config(page_title='小朋友', page_icon='👽')

cookie_manager = stx.CookieManager()
cookie = Cookie(cookie_manager)
current_chat = ''
...
def init_sidebar():
    with st.sidebar:
        st.header('👻 模型')
        st.selectbox("选择模型：", index=0, options=['gpt-3.5-turbo'], key="select_model", label_visibility='collapsed')

        st.checkbox('🎶 逐字回复', value=0, key='stream_mode')
        st.checkbox('↕️️ 携带上下文', value=0, key='with_context')
        st.checkbox('🕶️ 专家模式', value=0, key='expert_mode')


@handle_exception
def main():
    st.sidebar.title('小朋友')

    user_api_key = st.session_state.get(Auth.st_auth_api_key, '')
    if user_api_key:
        if Auth.valid_api_key(user_api_key):
            st.session_state[Auth.state_valid_api_key] = user_api_key
        else:
            st.session_state[Auth.state_valid_api_key] = ''

    user_password = st.session_state.get(Auth.st_auth_password, '')
    if user_password:
        if Auth.match_password(user_password):
            st.session_state[Auth.state_valid_password] = user_password
        else:
            st.session_state[Auth.state_valid_password] = ''

    status = Auth.login(cookie, st.session_state)
    if not status:
        st.error('请先认证，三选一')
        return
    ...
    def write_data(new_chat_name=current_chat):
        if "apikey" in st.secrets:
            st.session_state["paras"] = {
                "temperature": st.session_state.get("temperature" + current_chat, 4),
                "top_p": st.session_state.get("top_p" + current_chat, 1),
                "presence_penalty": st.session_state.get("presence_penalty" + current_chat, 0),
                "frequency_penalty": st.session_state.get("frequency_penalty" + current_chat, 0),
            }
    ...
    with st.container():
        ...
        with st.form("input_form", clear_on_submit=True):
            # 这个label不能随便改，会影响自动朗读
            user_input = st.text_area("**我说：**", key="user_input_area",
                                      help="内容将以Markdown格式在页面展示，建议遵循相关语言规范，同样有利于GPT正确读取，例如："
                                           "\n- 代码块写在三个反引号内，并标注语言类型"
                                           "\n- 以英文冒号开头的内容或者正则表达式等写在单反引号内",
                                      value=st.session_state['user_voice_value'])
            submitted = st.form_submit_button("🚀 提交", use_container_width=True, on_click=input_callback)
        if submitted:
    ...
    if st.session_state.get('expert_mode', 0):
        tap_context, tap_model, tab_func = st.tabs(['🗒️ 预设', '⚙️ 模型', '🛠️ 功能'])
        with tap_context:
    ...
    def get_model_input():
        # 需输入的历史记录
        history = initial_content_history + [{"role": "user", "content": st.session_state['pre_user_input_content']}]
        if st.session_state.get('with_context', 0):
            context_level = st.session_state.get('context_level' + current_chat, 4)
            history = (get_history_input(st.session_state["history" + current_chat], context_level) +
                       [{"role": "user", "content": st.session_state['pre_user_input_content']}])
            for ctx in [st.session_state.get('context_input' + current_chat, ''),
                        set_context_all[st.session_state.get('context_select' + current_chat, '不设置')]]:
                if ctx != "":
                    history = [{"role": "system", "content": ctx}] + history

        # 设定的模型参数
        paras = {
            "temperature": st.session_state.get("temperature" + current_chat, 1),
            "top_p": st.session_state.get("top_p" + current_chat, 1),
            "presence_penalty": st.session_state.get("presence_penalty" + current_chat, 0),
            "frequency_penalty": st.session_state.get("frequency_penalty" + current_chat, 0),
        }
    ...
    if st.session_state['user_input_content'] != '':
        ...
        with running_flag:
            with st.spinner("🤔"):
                api_key = Auth.get_api_key(cookie, st.session_state)
                ChatGPT.chat(
                    api_key, st.session_state, history_need_input, paras_need_input, current_chat, area_error,
                    stream_mode=st.session_state.get('stream_mode', False)
                )
